#!/usr/bin/env python3
"""Defect register with P0–P3 severity for MIT-Test.
...
from mit_lib import DEFECT_LEVELS, ensure_mit, load_state, read_json, save_state, utc_now, write_json  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    mit = ensure_mit(root)
    gate_path = mit / "defects" / f"{defect_id}.fix_gate.json"
    ...
    out = mit / "defects" / f"{defect_id}.json"
    write_json(out, defect)

    if state is not None:
        counts = {"P0": 0, "P1": 0, "P2": 0, "P3": 0}
        for p in (mit / "defects").glob("*.json"):
