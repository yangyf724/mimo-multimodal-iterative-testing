#!/usr/bin/env python3
"""Build release candidate metadata (RC) for MIT-Test."""
...
from mit_lib import ensure_mit, load_state, save_state, sha256_file, utc_now, write_json  # noqa: E402


def hash_tree(root: Path, exclude: set[str] | None = None) -> str:
    exclude = exclude or {".git", ".mit", ".bug-hunter", "node_modules", "__pycache__", ".venv", ".worktrees"}
    h = hashlib.sha256()
    files: list[Path] = []
