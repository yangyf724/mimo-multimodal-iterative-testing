#!/usr/bin/env python3
"""Migrate .bug-hunter/state.json to .mit/state.json (MIBG → MIT-Test)."""
...
def migrate(old: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "version": 1,
        "skill": "multimodal-iterative-testing",
        ...
        "migrated_from": {
            ...
            "skill": old.get("skill", "iterative-bug-hunter"),
            "run_count": old.get("run_count"),
            "modalities_enabled": old.get("modalities_enabled"),
            "migrated_at": now,
        },
        "resume_summary": (
            f"migrated from MIBG run_count={old.get('run_count')} quiet_streak={old.get('quiet_streak')}"
        ),
    }


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Migrate MIBG .bug-hunter state to MIT .mit state")
