...
from mmit_lib import ensure_tests, ensure_mit, load_state, read_json, utc_now  # noqa: E402


def _load(root: Path, name: str) -> Any:
    p = ensure_tests(root) / name if name in {"profile.json", "matrix.json", "prod_test_results.json", "data_mask_report.json", "env_manifest.json"} else ensure_mit(root) / name
    if p.exists():
        return read_json(p)
    return {}
...
def build_report(root: Path) -> str:
    ...
    lines += [
        ...
        "## 10. 证据包索引",
        "",
        "- 见 `.mmit/deliverables/EVIDENCE_INDEX.md` 与 `.mmit/deliverables/evidence/`。",
        "",
    ]
    return "\n".join(lines)
