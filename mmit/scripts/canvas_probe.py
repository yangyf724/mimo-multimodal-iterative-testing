...
def _coerce_size(value: Any) -> str | None:
    """Accept '1200x628', {width,height}, (w,h) or {w,h}."""
    if value is None:
        return None
...
def normalize_item(raw: dict[str, Any], *, default_id: str | None = None) -> dict[str, Any]:
    item_id = raw.get("id") or raw.get("scene_id") or default_id or "canvas-item"
    export_size = _coerce_size(raw.get("export_size")) or _coerce_size(raw.get("export_target"))
    if not export_size and raw.get("width") and raw.get("height"):
        export_size = f"{raw.get('width')}x{raw.get('height')}"
    spec = raw.get("spec") or {}
    objects = list(raw.get("objects") or [])
    assets = list(raw.get("assets") or [])
    # Map scene "layers" schema onto objects/assets (MIT scene format)
    for layer in raw.get("layers") or []:
        if not isinstance(layer, dict):
            continue
        if layer.get("type") == "image" or layer.get("src"):
            assets.append(
                {
                    "id": layer.get("id"),
                    "src": layer.get("src"),
                    "w": layer.get("w") or layer.get("width"),
                    "h": layer.get("h") or layer.get("height"),
                    "dpi": layer.get("dpi"),
                    "display_w": layer.get("w") or layer.get("width"),
                    "display_h": layer.get("h") or layer.get("height"),
                }
            )
        objects.append(layer)
    export_target = _coerce_size(spec.get("export_target")) or _coerce_size(raw.get("export_target"))
    return {
        "id": str(item_id),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.items:
        ...
        mit_state = root / ".mit" / "state.json"
        state: dict[str, Any] = {}
        if state_path.exists():
            with state_path.open("r", encoding="utf-8-sig") as f:
                state = json.load(f)
        elif mit_state.exists():
            with mit_state.open("r", encoding="utf-8-sig") as f:
                state = json.load(f)
            # MIT layout: discover canvas scenes under canvas/
            scene_dir = root / "canvas"
            if scene_dir.is_dir():
                for sp in sorted(scene_dir.glob("*.scene.json")):
                    items.extend(load_items_from_path(sp))
        else:
            print(json.dumps({"ok": False, "error": "state.json missing"}, file=sys.stderr, ensure_ascii=False))
            return 1
        ...
        if not items:
            # discover canvas scenes under canvas/ (unified + legacy layouts)
            for scene_dir in (root / "canvas", root / ".mmit" / "canvas"):
                if scene_dir.is_dir():
                    for sp in sorted(scene_dir.glob("*.scene.json")):
                        items.extend(load_items_from_path(sp))
    ...
    if rules:
        ...
        result = {"ok": len(findings) == 0, "findings": findings, "unavailable": unavailable}
    else:
    ...
    # Honest exit: findings fail; zero real checks (all unavailable) also fail — never green empty.
    checked = int(result.get("items_scanned") or 0) - len(unavailable)
    result["checked_items"] = max(checked, 0)
    if findings:
        result["ok"] = False
        result["error"] = f"{len(findings)} finding(s)"
        exit_code = 1
    elif not items:
        result["ok"] = False
        result["error"] = "no canvas items to check"
        exit_code = 1
    elif result["checked_items"] <= 0:
        result["ok"] = False
        result["error"] = "no item produced real checks (all unavailable)"
        exit_code = 1
    else:
        result["ok"] = True
        exit_code = 0
