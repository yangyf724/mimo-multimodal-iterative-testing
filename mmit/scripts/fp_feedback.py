...
def default_patterns_path(root: Path) -> Path:
    return root / ".mmit" / "fp_patterns.json"
...
def agents_snippet(store: dict[str, Any]) -> str:
    ...
    lines = [
        "# mmit FP whitelist (generated)",
        "",
        "Do **not** auto-edit this project's AGENTS.md from this snippet without review.",
        "",
        f"- Patterns file: `.mmit/fp_patterns.json` ({len(patterns)} pattern(s))",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    snip.add_argument("--out", default="AGENTS.mmit.snippet.md")
