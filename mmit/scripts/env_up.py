#!/usr/bin/env python3
"""Environment manifest for MMIT-Test (E0–E3)."""
...
from mmit_lib import ensure_tests, ensure_mit, read_json, utc_now, write_json  # noqa: E402


def build_env_manifest(root: Path, prod_access: str = "none") -> dict[str, Any]:
    profile_path = ensure_tests(root) / "profile.json"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Write env_manifest.json for MMIT-Test")
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = ensure_tests(root) / "env_manifest.json"
