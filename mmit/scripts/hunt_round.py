...
def run_hunt_round(
    *,
    ...
    captures_dir = captures or (root / ".mmit" / "hunts" / "runs" / run_id / "captures")
    findings: list[dict[str, Any]] = []
    ...
    if canvas_scanned:
        ...
        if canvas_findings:
            save_json(
                root / ".mmit" / "hunts" / "runs" / run_id / "findings" / "raw" / "canvas__items.json",
        ...
        save_json(
            root / ".mmit" / "hunts" / "runs" / run_id / "findings" / "raw" / "canvas__unavailable.json",
    ...
    patterns_path = fp_patterns or (root / ".mmit" / "fp_patterns.json")
    ...
    if patterns_path.exists():
        try:
            ...
            suppressed_hits = []
            save_json(
                root / ".mmit" / "hunts" / "runs" / run_id / "findings" / "fp-apply-error.json",
    ...
    if write_candidates:
        cand_dir = root / ".mmit" / "hunts" / "runs" / run_id / "findings" / "candidates"
    ...
    save_json(root / ".mmit" / "hunts" / "runs" / run_id / "summary.json", summary)
    return summary
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        dest="fp_patterns",
        default=None,
        help="FP whitelist JSON (default .mmit/fp_patterns.json)",
    )
