...
def _hunter_main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        out_default = root / ".mmit" / "hunts" / "runs" / str(run_id) / "fix-verify.json"
...
def evaluate_cli(argv=None) -> int:
    p = argparse.ArgumentParser(description="Evaluate Fix Gate four conditions (MMIT)")
    p.add_argument("--root", type=Path, default=Path.cwd())
    p.add_argument("--defect-id", required=True)
    p.add_argument("--target-cleared", action="store_true")
    p.add_argument("--zero-new-high", action="store_true")
    p.add_argument("--non-target-stable", action="store_true")
    p.add_argument("--regression-green", action="store_true")
    args = p.parse_args(argv)
    root = args.root.resolve()
    try:
        defect_id = sanitize_id(args.defect_id, "defect-id")
    except ValueError:
        return 2
    ...
    out = root / ".mmit" / "tests" / "defects" / f"{defect_id}.fix_gate.json"
...
def main(argv=None) -> int:
    ...
    if "--defect-id" in argv:
        return evaluate_cli(argv)
    return _hunter_main(argv)
