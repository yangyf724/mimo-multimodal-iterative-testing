...
from mmit_lib import ensure_mit, ensure_tests, load_state, read_json, save_state, utc_now  # noqa: E402


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build .mmit/escalation/ package")
...
def main(argv: list[str] | None = None) -> int:
    ...
    prod = tests / "prod_test_results.json"
    if prod.exists():
        shutil.copy2(prod, esc / "prod_test_results.json")

    open_defects = []
    defects_dir = tests / "defects"
    if defects_dir.is_dir():
        for p in defects_dir.glob("*.json"):
            ...
            try:
                ...
                continue
    from mmit_lib import write_json
