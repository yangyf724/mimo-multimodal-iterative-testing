...
3. compose 的 implement/verify/review 遵守 compose-next；**禁止** review/implement subagent 写 `.mmit/state.json`（单写者）。  
...
路径建议：`.mmit/bugs/<bug-id>/packet.json`（main agent 持锁写 `bugs/**`）。
...
- subagent 写 `.mmit` state / fingerprints / bugs  
