...
Default path: `.mmit/export/report.json`
...
python scripts/export_report.py --validate .mmit/export/report.json
```

Validate exit `0` when schema checks pass; unknown `schema_version` → exit `3`; other validation errors → exit `1`.

REPORT.md remains the human document; export is a parallel machine view derived from the same `.mmit` state.
