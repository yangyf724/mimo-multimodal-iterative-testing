...
Flow 文件（运行时默认 `.mmit/flows/*.json`；验收 demo 固定在 `examples/acceptance-demo/flows/`，避免被 `.mmit/` ignore 掉）：
...
ux_flow.py --manifest runs/run-N/captures/MANIFEST.json --flows .mmit/flows --out findings.json
