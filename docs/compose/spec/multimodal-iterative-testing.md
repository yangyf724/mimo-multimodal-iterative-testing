---
feature: multimodal-iterative-testing
status: delivered
updated: 2026-09-25
branch: dev
...
# 全模态迭代测试 Skill 重构（MIT-Test）

## Report

**What was built** — 新 skill 包 `multimodal-iterative-testing/`（MIT-Test）：SKILL.md + locales + 24 篇 references + 25 个脚本（画像/矩阵/状态/RC/矩阵执行/缺陷/修复门/脱敏/环境/设备/生产/交付装配/升级/迁移，及 MIBG 探针迁入副本）。核心契约落地为：Profile→Matrix 冻结（`matrix_rev`）、`.mit/` 状态机与 RC 原子性、P0–P3 + Fix Gate 四条件（无回归不得 fixed）、脱敏硬门（matrix_run 拒跑脏报告）、E0–E3 / PRD 缺失声明、TEST_REPORT 十节 + 证据包 + 裁决信号 + Allow/Deny/Escalate。SIG 不再假绿：`regression_green` 由缺陷/门推导，`artifact_ready` 要求 required 覆盖完整；探针禁止 `--help`/空跑假 PASS。

双 demo：`examples/mit-demo-webapi/`（Deny 路径）与 `examples/mit-demo-media/`（Escalate + 升级包）。DoD §17.1–6 演示通过；§17.7 经评审修复后关闭（canvas 真实检查、无空跑 PASS）。旧 MIBG 保留，description 负例指向 MIT-Test。
...
- `iterative-bug-hunter/scripts/ci_gate.py --root .` → **PASS**（ok:true EXIT=0；PRE-EXISTING Windows GBK 读线程噪音）
...
**Journey log**
1. MIT/MIBG 脚本同名（`init_state`/`fix_gate`）在 `unittest discover` 共享 `sys.modules` 会串包 → 测试侧 `importlib` 以 `mit_*` 唯一名加载。
...
5. Windows 子进程读 UTF-8 须 `encoding="utf-8", errors="replace"`，否则 GBK 解码炸线程（MIBG `ci_gate` 仍有 PRE-EXISTING 噪音）。

## [S1] Problem

`iterative-bug-hunter`（MIBG）只覆盖 code / web-visual / canvas 三通道并以 quiet 收敛为出口，无法回答「这个候选版本能不能正常发布使用」。差距见设计真源 `docs/skill-refactor-multimodal-iterative-testing.md` §1.2（G1–G7）：模态漏测、本地环境不等价、无 RC 语义、缺陷无发布分级、交付物缺测试报告/证据包/裁决信号、无生产侧协议、裁决不可综合。

本 feature 将 MIBG 重构为 **全模态迭代测试** skill（`multimodal-iterative-testing` / MIT-Test），按项目画像生成全模态测试矩阵，在生产等价环境与脱敏数据下迭代执行发布就绪测试，分级修复并强制回归，每轮重建 RC 全量复测，最终由 agent 综合四项依据裁决 Allow / Deny / Escalate。

## [S2] Design

设计真源：`docs/skill-refactor-multimodal-iterative-testing.md`（MIT-DESIGN-001 v1.0）。本节只记录落地契约；与真源冲突时以真源的目标、裁决原则与禁止项为准。

### 命名与包布局

| 项 | 值 |
|----|-----|
| 英文 ID / 目录 | `multimodal-iterative-testing/`（仓库根，与 `iterative-bug-hunter/` 并列） |
| 中文显示名 | 全模态迭代测试 |
| 状态目录 | 目标项目 `.mit/` |
...
multimodal-iterative-testing/
  ...
  references/*.md          # 24 篇
  scripts/*.py             # 见任务表；含自 MIBG 迁入的探针副本
```

旧包 `iterative-bug-hunter/` 保留为视觉/布局抓 BUG 入口；`SKILL.md` description 增补负例指向 MIT-Test。提供 `scripts/migrate_state.py` 将 `.bug-hunter/state.json` 迁到 `.mit/state.json`。
...
- 未授权生产写路径压测或破坏性数据操作。
- 本 feature 不删除 `iterative-bug-hunter` 包，不迁移已安装到 `~/.config/mimocode/skills/` 的运行时副本（安装留给后续发布流程）。
...
- [x] T1: SKILL 包骨架 — acceptance: `multimodal-iterative-testing/` 含 SKILL.md（单行 frontmatter）+ locales 显示名正确 + 空 references/scripts 目录约定成立 (covers: S2)
...
- [x] T10: 自 MIBG 迁入探针副本并接线 — acceptance: layout/contrast/ux/canvas/vlm/visual_diff 等可在 matrix_run 下被调用；指纹/FP 白名单可用；canvas 支持 `.mit`+layers (covers: S2; depends: T6)
...
- [x] T13: 旧 skill 负例与 CHANGELOG/README 对齐 — acceptance: MIBG description 负例指向 MIT-Test；README/CHANGELOG 描述新包；旧测试仍绿 (covers: S2; depends: T1)
