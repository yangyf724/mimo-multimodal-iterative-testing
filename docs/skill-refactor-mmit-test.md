...
| 版本 | v1.0 |
| 状态 | 设计定稿（待实现） |
| 源 Skill | `mmit-hunter`（迭代式抓 BUG / MMIT） |
| 目标 Skill | **全模态迭代测试**（`mmit-test`，简称 **MMIT-Test**） |
...
`mmit-hunter` 面向「抓 BUG 直到收敛」：以代码、Web 视觉、画布三通道为主，输出缺陷报告。该模型适合设计 QA 与缺陷狩猎，但**不能直接回答发布问题**：
...
| 中文显示名 | **全模态迭代测试** |
| 英文 ID | `mmit-test` |
| 对话简称 | MMIT-Test（避免与许可证 MIT 混淆时写全名） |
| 状态目录 | `.mit/` |
...
2. 纯功能开发或新接口实现。
3. 仅视觉/布局抓 BUG（继续使用 `mmit-hunter`）。
...
| 能力 | 处理 |
|------|------|
| Web 视觉/无障碍/布局探针、canvas 探针 | **复用** MMIT 实现 |
...
## 14. 重构映射（MMIT → MMIT-Test）

| 源（mmit-hunter） | 目标（mmit-test） | 策略 |
|----------------------------|--------------------------------------|------|
...
| 与 MMIT 边界模糊 | 触发条件负例写清；视觉-only 场景继续走 MMIT |
...
| 内部 | MMIT DESIGN/fix-gate/confirm 协议 | 指纹、确认门、回归门 |
